...
> {
  ...
  private getMetrics(entity: string, timeRange: string) {
    return {
      ...
      total_operations: {
        query: `sum(weliv_entity_operations_total{entity="${entity}"})`,
        prometheus_url: `http://localhost:19090/api/v1/query?query=sum(weliv_entity_operations_total{entity="${entity}"})`,
      },

      // Taxa de operações por minuto
      operations_rate: {
        query: `rate(weliv_entity_operations_total{entity="${entity}"}${timeRange})`,
        prometheus_url: `http://localhost:19090/api/v1/query?query=rate(weliv_entity_operations_total{entity="${entity}"}${timeRange})`,
      },

      // Duração média das operações
      avg_duration: {
        query: `rate(weliv_operation_duration_seconds_sum{entity="${entity}"}${timeRange}) / rate(weliv_operation_duration_seconds_count{entity="${entity}"}${timeRange})`,
        prometheus_url: `http://localhost:19090/api/v1/query?query=rate(weliv_operation_duration_seconds_sum{entity="${entity}"}${timeRange})/rate(weliv_operation_duration_seconds_count{entity="${entity}"}${timeRange})`,
      },

      // Operações por status
      by_status: {
        query: `sum(weliv_entity_operations_total{entity="${entity}"}) by (status)`,
        prometheus_url: `http://localhost:19090/api/v1/query?query=sum(weliv_entity_operations_total{entity="${entity}"}) by (status)`,
      },

      // Operações por ação
      by_action: {
        query: `sum(weliv_entity_operations_total{entity="${entity}"}) by (action)`,
        prometheus_url: `http://localhost:19090/api/v1/query?query=sum(weliv_entity_operations_total{entity="${entity}"}) by (action)`,
      },

      info: {
        message: 'Métricas agora vêm do Prometheus',
        grafana_dashboard: 'http://localhost:30101',   // weliv: bloco 30xxx
        prometheus_ui: 'http://localhost:19090',  // weliv: bloco 19xxx
      },
    };
  }
}
