  ...
  implements ExceptionFilter
{
  ...
  private handleValidationError(
    ...
    // Para erros do Prisma, usar apenas a mensagem amigável
    this.sendErrorResponseForPrisma(
      exception,
      host,
  ...
  private handleKnownRequestError(
    ...
    // Para erros do Prisma, usar apenas a mensagem amigável (não a mensagem técnica do exception)
    this.sendErrorResponseForPrisma(exception, host, status, errorCode, message);
  }

  /**
   * Sobrescreve sendErrorResponse para erros do Prisma
   * Usa apenas a mensagem amigável, ignorando a mensagem técnica do exception
   */
  private sendErrorResponseForPrisma(
    exception: any,
    host: ArgumentsHost,
    status: HttpStatus,
    errorCode: string,
    ...
    // Log do erro para debug interno (usa a mensagem técnica para logs)
    this.logger.error(
    ...
    // Resposta padronizada usando APENAS a mensagem amigável
    const errorResponse = {
      error: errorCode,
      message: message, // Usa apenas a mensagem amigável, não exception.message
    };
  ...
  /**
   * Retorna nome amigável do campo para exibição
   */
  ...
  private mapPrismaErrorCode(
    ...
    switch (code) {
      ...
      case 'P2002':
        // Erro de constraint única - identificar qual campo está duplicado
        const targetFields = meta?.target || [];
        const fieldName = targetFields.length > 0 ? targetFields[0] : null;
        ...
        if (fieldName) {
          switch (fieldName.toLowerCase()) {
            case 'cnpj':
              message = VALIDATION_MESSAGES.UNIQUENESS.CNPJ_EXISTS;
              break;
            case 'cpf':
              message = VALIDATION_MESSAGES.UNIQUENESS.CPF_EXISTS;
              break;
            case 'email':
            ...
            case 'login':
              ...
              break;
            default:
              // Mensagem genérica com nome do campo
              const fieldDisplayName = this.getFieldDisplayName(fieldName);
              message = `${fieldDisplayName} já está cadastrado no sistema`;
          }
        }
