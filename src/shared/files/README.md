...
Sistema simples de armazenamento de arquivos integrado ao backend NestJS do TemplateLobocode Engine, utilizando MinIO como storage.
...
### 2. Docker Compose
O MinIO já está configurado no `docker/docker-compose.vps-app.yml`:
```yaml
minio:
...
### 1. Iniciar Serviços
```bash
# Iniciar apenas MinIO no stack atual
